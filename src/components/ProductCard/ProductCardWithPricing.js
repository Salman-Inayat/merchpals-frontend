...
const ProductCard = ({
  ...
  const renderBgColor = () => {
    ...
    if (selectedVariants[product.productId]) {
      const productSelectedVariants = selectedVariants[product.productId];
    ...
    <>
      ...
      >
        <Box className={classes.checkboxContainer}>
          <Checkbox
            checked={selectedVariants[product.productId] ? true : false}
            onChange={(e) => onProductClick(e.target.value)}
          ...
          product.colors.map((pm, i) => (
            <Grid key={`colors-${i}`} item md={2} xs={2}>
              <Checkbox
                ...
                checked={
                  selectedVariants[product.productId]?.includes(pm.id) ? true : false
                }
                value={`${product.productId},${pm.id}`}
                sx={{
