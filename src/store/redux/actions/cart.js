...
export const login = (phoneNo, password) => async dispatch => {
  axios
    ...
    .catch(err =>
      dispatch({ type: AUTH_LOGIN_FAILED, message: err.response.data.message }),
    );
};
...
export const getLoggedInUserInfo = () => async dispatch => {
  axios
    ...
    .then(res =>
      dispatch({ type: GET_LOGGED_IN_USER_INFO_SUCCESS, payload: res.data }),
    )
    .catch(err =>
      dispatch({
        type: GET_LOGGED_IN_USER_INFO_FAILED,
